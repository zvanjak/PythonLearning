...
print(s1 + s2)               # the concatenation of s1 and s2
print(s * n, n * s)          # n copies of s concatenated
print(s[i])                  # i'th item of s, origin 0  (1)
print(s[i:j])                # Slice of s from i (included) to j (excluded). Optional step value, possibly negative (default: 1).
print(s[i:j:step])
...
print(reversed(s))           # [2.4] Returns an iterator on s in reverse order. s must be a sequence, not an iterator (use reversed(list(s)) in this case. [PEP 322]
